(() => {
  ...
  const menu=[[route('ru/#learning'),'Обучение'],[route('equipment-ru/'),'Снаряжение'],[route('ru/#forecast'),'Прогноз'],[route('ru/#team'),'Команда'],[route('blog-ru/'),'Блог'],[route('ru/#faq'),'ЧЗВ'],[route('organizers-ru/'),'Организаторам'],['https://vetratoria.ru/','Виндсерфинг','special']];
  ...
  mount.innerHTML=`<header id="vf-fixed-menu"><div class="vf-desk-wrap"><a class="vf-desk-logo" href="${route('ru/')}"><img src="${asset('logo-ru-orange-v2.png')}" alt="Ветратория"></a><nav class="vf-desk-nav"><div class="vf-desk-nav-inner">${links}</div></nav><div class="vf-desk-right"><div class="vf-social">${socials}</div><div class="vf-lang-switcher"><a href="${route('equipment/')}" class="vf-lang-btn">EN</a><a href="${route('equipment-ru/')}" class="vf-lang-btn active">РУС</a></div><button class="vf-burger" type="button" aria-label="Открыть меню" aria-expanded="false"><span></span><span></span><span></span></button></div></div></header><div class="vf-panel" aria-hidden="true"><div class="vf-panel-inner"><div class="vf-panel-logo"><img src="${asset('logo-ru-orange-v2.png')}" alt="Ветратория"></div><nav class="vf-nav">${links}</nav><div class="vf-m-social">${socials}</div></div></div>`;
