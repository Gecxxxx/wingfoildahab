(() => {
  ...
  mount.innerHTML = `<section class="vf-blog" aria-labelledby="vf-blog-title"><div class="vf-blog__inner"><div class="vf-blog__head"><div><p class="vf-blog__eyebrow">Vetratoria Journal</p><h2 id="vf-blog-title">${ru ? 'Блог' : 'Blog'}</h2></div><p class="vf-blog__intro">${ru ? 'Разбираем обучение, снаряжение, безопасность и жизнь на воде — честно и на опыте инструкторов.' : 'Guides to learning, equipment, safety and life on the water — written from real instructor experience.'}</p></div><div class="vf-blog__grid">${posts.map(card).join('')}</div><div class="vf-blog__actions"><a class="vf-blog__all" href="${route('blog/')}">${ru ? 'Все статьи' : 'All articles'} →</a></div></div></section>`;
})();
