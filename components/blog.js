(() => {
  ...
  mount.innerHTML = `<section class="vf-blog" aria-labelledby="vf-blog-title"><div class="vf-blog__inner"><div class="vf-blog__head"><h2 id="vf-blog-title">${ru ? 'Блог' : 'Blog'}</h2></div><div class="vf-blog__grid">${posts.map(card).join('')}</div><div class="vf-blog__actions"><a class="vf-blog__all" href="${route(ru ? 'blog-ru/' : 'blog/')}">${ru ? 'Все статьи' : 'All articles'} →</a></div></div></section>`;
})();
