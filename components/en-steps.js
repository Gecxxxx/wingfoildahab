(() => {
  ...
  const steps = [
    ...
    { num:'Step 04', title:'Progression & Tricks', preview:'Master jibes, tacks, and riding in open water and waves.', img:'step-04.webp', full:'Once you can fly confidently, progress to smooth transitions, advanced maneuvers and your first open-sea wave sessions.', skills:['Jibes and tacks','Maintaining height through turns','Wave starts and riding technique'], note:'Goal: foil turns without losing speed and confident open-water riding.' },
  ];
  mount.innerHTML = `<section class="steps-section uc-steps" id="learning-stages"><div class="steps-container"><h2>Your path to the <span class="steps-orange">first flight</span></h2><div class="steps-grid">${steps.map((step,index)=>`<article class="step-card" data-step="${index}" tabindex="0"><div class="step-img"><img src="${asset(step.img)}" alt="${step.title}" loading="lazy" decoding="async"></div><div class="step-info"><span class="step-hint">Click for details →</span><span class="step-num">${step.num}</span><h3 class="step-title">${step.title}</h3><p class="step-preview">${step.preview}</p></div></article>`).join('')}</div><div class="steps-footer"><a href="#booking-form-en" class="steps-main-btn">Contact Us</a></div></div>
    <div class="steps-modal-overlay"><div class="steps-modal-content"><button class="steps-modal-close" type="button" aria-label="Close">×</button><div class="steps-modal-image"></div><div class="steps-modal-text"><h3></h3><h4></h4><div class="steps-modal-details"></div><div class="steps-modal-note"></div><ul class="steps-modal-list"></ul><a href="#booking-form-en" class="steps-modal-btn">Book this stage</a></div></div></div>
  </section>`;
