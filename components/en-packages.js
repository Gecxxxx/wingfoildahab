(() => {
  ...
  let current='lessons';
  const tabs=(modifier='')=>`<div class="packages-tabs ${modifier}" aria-label="Package type"><button class="packages-tab${current==='lessons'?' active':''}" data-tab="lessons" type="button" aria-pressed="${current==='lessons'}">Lessons Only</button><button class="packages-tab${current==='intensive'?' active':''}" data-tab="intensive" type="button" aria-pressed="${current==='intensive'}">Intensive (+ Rental)</button></div>`;
  mount.innerHTML=`<section class="wing-packages"><div class="packages-wrap"><h2>Training <span class="packages-orange">Packages</span></h2>${tabs('packages-tabs--primary')}<div class="packages-grid"></div></div><div class="package-modal" role="dialog" aria-modal="true"><div class="package-modal-content"><button class="package-modal-close" type="button" aria-label="Close">×</button><div class="package-modal-info"><h3></h3><p>A professional training course at our Dahab station with modern RRD equipment and experienced instructors.</p><ul class="package-modal-list"></ul><button class="package-btn package-primary package-modal-book" type="button">Book</button></div><div class="package-modal-image"></div></div></div></section>`;
  const grid=mount.querySelector('.packages-grid'),modal=mount.querySelector('.package-modal');
  const scrollForm=()=>document.querySelector('#booking-form-en')?.scrollIntoView({behavior:'smooth',block:'start'});
