(() => {
  ...
  mount.innerHTML=`<section class="wing-price-record uc-price"><div class="wing-price-list"><div class="price-wrap"><span class="price-kicker">Разовые занятия и прокат</span><h2>Цены</h2><div class="price-grid">${items.map((item,index)=>`<article class="price-card"><div class="price-thumb"><img src="${asset(item[0])}" alt="${item[1]}" loading="lazy"></div><div class="price-body"><h3>${item[1]}</h3><span class="price-meta">${item[2]}</span><div class="price-box"><span class="price-num">${item[3]}</span><span class="price-cur">${item[4]}</span></div><div class="price-actions"><a href="#" class="price-btn price-btn--primary" data-scroll=".uc-form">Записаться</a><button class="price-btn price-btn--ghost" type="button" data-price="${index}">Подробнее</button></div></div></article>`).join('')}</div><div class="price-cta"><p class="price-footnote">В стоимость урока включено: оборудование, страховка, шлем и жилет.</p><a class="price-equipment-link" href="https://wingfoildahab.com/equipment" target="_blank" rel="noopener">Цены на прокат оборудования</a></div></div><div class="price-modal-overlay"><div class="price-modal-content"><button class="price-modal-close" type="button" aria-label="Закрыть">×</button><div class="price-modal-image"></div><div class="price-modal-info"><h3></h3><span class="price-modal-meta"></span><p class="price-modal-text"></p><div class="price-actions"><a href="#" class="price-btn price-btn--primary" data-scroll=".uc-form">Записаться</a><button class="price-btn price-btn--ghost" type="button" data-close>Закрыть</button></div></div></div></div></div></section>`;
