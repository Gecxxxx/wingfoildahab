(() => {
  ...
  mount.innerHTML = `
    <section class="steps-section uc-steps" id="learning-stages"><div class="steps-container"><h2>Ваш путь к <span class="steps-orange">первому полету</span></h2><div class="steps-grid">${steps.map((step, index) => `<article class="step-card" data-step="${index}" tabindex="0"><div class="step-img"><img src="${asset(step.img)}" alt="${step.title}" loading="lazy"></div><div class="step-info"><span class="step-hint">Подробнее →</span><span class="step-num">${step.num}</span><h3 class="step-title">${step.title}</h3><p class="step-preview">${step.preview}</p></div></article>`).join('')}</div><div class="steps-footer"><a href="#booking-form" class="steps-main-btn" data-booking-service="wingfoil-lesson">Записаться на обучение</a></div></div>
      <div class="steps-modal-overlay" role="dialog" aria-modal="true"><div class="steps-modal-content"><button class="steps-modal-close" type="button" aria-label="Закрыть">×</button><div class="steps-modal-image"></div><div class="steps-modal-text"><h3></h3><h4></h4><div class="steps-modal-details"></div><div class="steps-modal-note"></div><ul class="steps-modal-list"></ul><a href="#booking-form" class="steps-modal-btn">Забронировать этот этап</a></div></div></div>
    </section>`;
  ...
  const open = (index) => { const step = steps[index]; mount.querySelector('.steps-modal-image').style.backgroundImage = `url('${asset(step.img)}')`; mount.querySelector('.steps-modal-text h3').textContent = step.num; mount.querySelector('.steps-modal-text h4').textContent = step.title; mount.querySelector('.steps-modal-details').textContent = step.full; mount.querySelector('.steps-modal-note').textContent = step.note; mount.querySelector('.steps-modal-list').innerHTML = step.skills.map(skill => `<li>${skill}</li>`).join(''); mount.querySelector('.steps-modal-btn').dataset.bookingService=`${step.num}: ${step.title}`; overlay.classList.add('active'); document.body.style.overflow = 'hidden'; mount.querySelector('.steps-modal-close').focus(); };
